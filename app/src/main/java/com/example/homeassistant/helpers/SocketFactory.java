...
public class SocketFactory extends javax.net.ssl.SSLSocketFactory {
    ...
    public SocketFactory(SocketFactoryOptions options) throws KeyStoreException, NoSuchAlgorithmException, IOException, KeyManagementException, java.security.cert.CertificateException, UnrecoverableKeyException {
        ...
        if (options.hasCaCrt()) {
            ...
            Log.w("cert", "Certificate Owner: " + ca.getSubjectDN().toString());
            Log.w("cert", "Certificate Issuer: " + ca.getIssuerDN().toString());
            Log.w("cert", "Certificate Serial Number: " + ca.getSerialNumber().toString());
            Log.w("cert", "Certificate Algorithm: " + ca.getSigAlgName());
            Log.w("cert", "Certificate Version: " + ca.getVersion());
            Log.w("cert", "Certificate OID: " + ca.getSigAlgOID());
            Enumeration<String> aliasesCA = caKeyStore.aliases();
            for (; aliasesCA.hasMoreElements(); ) {
                String o = aliasesCA.nextElement();
                Log.w("cert", "Alias: " + o + " isKeyEntry: " + caKeyStore.isKeyEntry(o) + " isCertificateEntry: " + caKeyStore.isCertificateEntry(o));
            }


        } else {
            Log.w("cert", "CA sideload: false, using system keystore");
